...
from emu.io import read_vol, compute_bbox_all, read_vol, write_h5
...
def seg_to_iou(seg0, seg1, uid0=None, bb0=None, uid1=None, uc1=None):
    ...
    """
    assert (
        np.array(seg0.shape) - seg1.shape
    ).abs().max() == 0, "seg0 and seg1 should have the same shape"
    if bb0 is not None:
        if seg0.ndim == 2:
            assert (
                bb0.shape[1] == 6
...
def segs_to_remapped(get_seg, index, mapping, global_id=None, get_filename=None):
    ...
    for i, ind in index:
        seg = get_seg(ind)
        seg_out = seg_to_remapped(seg + global_id[i], mapping)
