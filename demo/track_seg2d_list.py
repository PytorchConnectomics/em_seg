...
from seglib import seg_list_to_iou, iou_list_to_global_id, iou_list_to_matches, seg_list_to_remapped, merge_id
...
if __name__ == "__main__":
    ...
    index = sorted(
        [
            int(x[x.rfind("/") + 1 : -3])
            for x in glob.glob(os.path.dirname(result_seg2d))
        ]
    )
    ...
    if opt == "0":
        ...
        output_list = [output_seg % x for x in index_parallel]
